...
當一個運算元操作了不同型別的數值時，需要通過一些規則將其轉為為共同的型別。一般來說，自動轉換會將「空間佔用較小的」轉換為「空間佔用較大的」資料型態，這樣的轉換並不會喪失訊息。例如，假設我們有一個浮點數 `f` 與一個整數 `i`，在計算 `f+i` 的表達式時會將整數 `i` 自動轉換為浮點數。

針對可能會造成資料喪失的轉換，尤其是指「空間佔用大的」轉換為「空間佔用小的」資料型態（例如 `long long int` 轉為 `int`），編譯器可能會給出警告，但這些表達式是合法的（可能會造成預期外的結果，通常而言除非明確知道自己在做什麼，否則不推薦這麼做）。

由於 `char` 是較小的整數型態，因此在算術表達式中可以自由使用 `char` 類型的變數，這使得實現某些字元轉換提到了豐富的靈活性。例如，下面的 `atoi` 函式就是一例，它會將數字字串轉換為相應的整數。
...
我們曾在 [第一章](./ch-01-introduction.md) 提過：`s[i] - '0'` 能夠計算出 `s[i]` 中儲存的字元所對應的數值，這是因為 `'0'`、`'1'` 等字元在 ASCII 字元集中是一個連續的遞增數列。

在 `n*10 + (s[i]-'0')` 中，`n*10` 是一個 `int` 型態、`(s[i]-'0')` 則是 `char` 型態，自動型別轉換的規則會將後者轉換為 `int` 型態並計算加總。

函式 `lower` 則是將 `char` 類型轉換為 `int` 類型的另一個例子，它將 ASCII 字元集中的字元映射到對應的小寫字母，如果給定的字元並非大寫字母，則會保持原始字元不變。
...
在 `ctype.h` 標頭檔中定義了一組測試與轉換字元用的函式。例如 `tolower(c)` 函式將 `c` 轉換為小寫（如果給定的 `c` 為大寫形式），可以使用 `tolower` 替代上述實作的 `lower(c)` 函式；同樣地，可以使用標準庫中的 `isdigit(c)` 替代上述的 `c >= '0' && c <= '9'`，在本書的後續內容中我們都將使用 `ctype.h` 所定義的函式。