...
在第 3 行，我們率先定義了 _函式原型（Funcion Prototype）_，用於提示接下來在 `main` 函式使用時的函式返回值及參數列表：
...
對於 `power` 的計算結果，會透過 `return` 陳述式將其返回給 `main` 函式。

```c
return p;
```

對於存在返回型態的函式來說，都應該使用 `return` 陳述式將指定的值返回給調用函式的地方，然而並不是所有函式都存在返回型態，可以使用 `return ;` 表達函式已經到了盡頭但不返回任何有用的值。

你可能注意到了，`main` 函式有被定義返回值 `int`，但是上述的所有程式都沒有在 `main` 函式的末尾加上 `return` 陳述式。這是因為 `main` 函式較為特殊，規範並不要求它一定要顯式使用返回值，這種情況會自動地 `return 0`。一般來說，`main` 函式的返回值為 0 時表示程式正常中止，反之則表示出現異常情況或出錯。
