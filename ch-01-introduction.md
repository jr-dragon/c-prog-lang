...
某些情況下可以省略 `extern`，在原始碼中如果全域sispmgp的定義出現在使用它的函式之前，那函式就沒必要使用 `extern`。因此，上述函式 `main`、`get_line` 及 `copy` 中的幾個 `extern` 都是多餘的。在習慣上，所有全域變數都會被定義在原始碼的開始處，這樣就可以省略 `extern`。
