...
在 C 語言中，可以使用 `#define` 來定義 _符號常數（Symbol Constant）_，在上述程式中我們定義了三個符號常數：
...
編譯器在 _前置處理（Preprocessing）_ 的過程中，會將符號常數填入回程式碼中，也就是說上述程式碼完全等價於以下程式碼，程式設計師往往會為了可讀性與可複用性來使用 `#define` 來定義常數。
...
在第 15 行中，使用了 `||` 運算元代表「或」，因此 `if (c == ' ' || c == '\n' || c == '\t')` 可以被口語地解釋為「如果 `c` 是空格，或 `c` 是換行字元，或 `c` 是製表字元」（前面有提到，`\t` 是製表字元的可見表示形態）。
...
在上一小節中，`main` 函式中的變數（如 `line` 或 `longest` 等）是函式中的局部變數，由於它們是在 `main` 函式中被宣告，因此 `get_line` 及 `copy` 等其它函式不能直接讀取或寫入它們。

由於局部變數僅在函式調用執行期間存在，因此在兩次的函式調用期間局部變數不會保留上次調用的賦值結果，需要在每次進入函式時都顯式為其賦值。如果局部變數沒有賦值，則其中存放的會是無效值。

除了局部變數，程式設計師還可以定義位於所有函式外部的變數，換句話說，在所有的函式中都可以訪問這些變數，它們被稱為全域變數。全域變數可以在程式的所有範圍存取（即便在不同的原始碼檔案中），函式可以透過全域變數來交換資料而不需要使用參數，且全域變數在程式執行期間會一直存在，即使在函式返回之後該值仍保持不變。

全域變數需要定義在所有函式之外，且只能定義一次。在每個需要存取外部變數的函式中，必需宣告相應的外部變數並說明其資料型態。宣告時可以用 `extern` 顯式宣告或通過上下文隱式宣告。

為了更詳細地討論全域變數，我們改寫 1.7 節中提到的輸出最長行程式，將 `line`、`longest` 與 `max` 宣告為全域變數。
...
在這個例子中，我們利用全域變數讓 `main`、`get_line` 及 `copy` 函式共享這些全域變數。從語法的角度來看，全域變數的定義與局部變數是相同的，只不過因為他們宣告於函式之外，所以它們是全域變數。

函式在使用全域變數的時候，只要如同往常一樣宣告變數，只不過在前面加上 `extern` 關鍵字即可。

某些情況下可以省略 `extern`，在原始碼中如果全域變數的定義出現在使用它的函式之前，那函式就沒必要使用 `extern`。因此，上述函式 `main`、`get_line` 及 `copy` 中的幾個 `extern` 都是多餘的。在習慣上，所有全域變數都會被定義在原始碼的開始處，這樣就可以省略 `extern`。

如果原始碼存在多個不同的檔案，某個全域變數僅在 `file1` 檔案中定義，而在 `file2` 及 `file3` 檔案使用。那麼在 `file2` 及 `file3` 就會需要使用 `extern` 來建立該變數與定義之間的聯繫。

程式設計師通常把變數與函式的 `extern` 宣告放在一個單獨的文件中（習慣上，這種文件稱為 _標頭檔（Head File）_），並在每個需要使用的原始檔用 `#include` 陳述式將它們引入。標頭檔通常會用 `.h` 作為其副檔名。例如，標準函式庫中的函式就位於類似 `stdio.h` 這樣的標頭檔中宣告的。

順帶一提，有部份的程式設計師會把用到的所有東西都作為全域變數；然而，過度依賴全域變數會導致一定的風險：它會使程式中的資料關係模糊不清，因為全域變數的值可能被意外地或不經意地修改，這使得維護難度陡升。

在 1.8 節中撰寫的使用全域變數的版本其實是較負面的示範，因為它使用了全域變數，且 `get_line` 與 `copy` 函式僅作為全域變數的存取器，從而使這兩個函式失去通用性。

## 1.9 結語

截至目前為止，我們已經對 C 語言的傳統核心部份進介扼要的介紹，藉由以上這些少量語言特性與元素，我們已經有能力編輯具有相當規模的程式。

建議讀者可以自行嘗試編寫一些程式，藉此更加熟悉 C 語言的特性與培養自己對於「優良程式的品味」 的嗅覺。